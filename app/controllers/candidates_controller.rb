...
autocomplete :candidate,:title,full:true,limit:10,:scopes=>[:unique_title]
  ...
  def group_email
    ...
    @candidates = @candidates.where("id > 16723")
	...
	puts "候选人的方向#{f.title}"
      ...
      job_pool.each do |job|
        if f.notified_jobs.where(job_id:job.id).empty?  and (f.updated_at < Time.now.days_ago(1))#该名候选人没有发送过这个职位并且发送职位间隔大于2天
          if (f.match(job) and (f.base_salary.nil? || job.base_pay.nil? || f.base_salary <= job.base_pay || job.base_pay == 0 || f.base_salary == 0 ))
  ...
  private
    ...
    def candidate_params
      params.require(:candidate).permit(:tag_list,:tag1,:tag2,:tag3,:base_salary,:year,:age,:name, :title, :employer, :mobile, :email, :city, :note)
    end
end
