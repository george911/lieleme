...
#  load_and_authorize_resource
  ...
  def show
    ...
    if current_user != nil
     ...
     if current_user
	@qr = RQRCode::QRCode.new( "http://123.57.191.137/cn/jobs/#{@job.id}?utf8=✓&forwarder=#{current_user.id}", :size => 8, :level => :h )
     end
 ...
 def apply
  ...
  private
    ...
    def job_params
      params.require(:job).permit(:title, :interview,:employer,:addr, :city, :industry, :commission, :role, :requirement, :base_pay, :month, :bonus, :allowance, :stock, :stock_num, :concall_date, :user_id, :peer, :memo, :company_info, :work_year,
