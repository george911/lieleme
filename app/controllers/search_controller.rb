class SearchController < ApplicationController
  ...
  def search
    case params[:q]
    when "人才","猎头"
     # authorize! :read,@search #@search要和ability.rb里定义的一致
  ...
  def job_search # 自动运行job_search.js.erb
    @user = User.find(session[:talent_id]) # session[:talent_id]来自users_controller的refer method,代表正在寻找匹配职位的候选人id.
    jobs = Job.all # 这个jobs要在循环外定义,否则会不认
    
    # 筛选出候选人没有应聘过的职位
    line_items = LineItem.where(recipient_id: session[:talent_id])
