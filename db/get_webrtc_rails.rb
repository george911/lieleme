...
["Ruby"].each do |language|
  [	  "Shanghai" 
	  ].each do |location|
    url1 = "https://github.com/search?utf8=%E2%9C%93&q=webrtc+location%3AShanghai+language%3Aruby&type=Repositories&ref=searchresults"
    ...
    (1..pages).each do |_id|
      puts "输出第#{_id}页"
      url = "https://github.com/search?utf8=%E2%9C%93&q=webrtc+location%3AShanghai+language%3Aruby&type=Repositories&ref=searchresults"
	...
	doc.css(".user-list-info").each do |item|
	  ...
	  if profile.css(".vcard-details").present?
	    ...
	    Candidate.create(user_id:10000,followers:followers,year:year,employer:employer,name:item.text.split.second,
			     title:(language=="C%23" ? "C#" : "Webrtc-Rails"),email:email_text,city:convert(location))
